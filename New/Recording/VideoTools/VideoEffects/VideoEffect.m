...
- (void) initializeVideo:(NSURL*) inputMovieURL fromSystemCamera:(BOOL)fromSystemCamera
{
    ...
    // 2. Add filter effect
    _filter = nil;
    NSUInteger themesCount = [[[VideoThemesData sharedInstance] getThemesData] count];
...
//    _movieFile.audioEncodingTarget = _movieWriter;
    ...
    NSLog(@"看看还在不在%@:",_movieWriter);
    NSLog(@"看看还在不在%@:",_movieFile);
    ...
    [_movieWriter setCompletionBlock:^{
        ...
        {
            [strongSelf -> _movieFile removeTarget:strongSelf -> _movieWriter];
        }
        
        [strongSelf -> _movieWriter finishRecordingWithCompletionHandler:^{
            // Closer timer
            [_timerFilter invalidate];
            _timerFilter = nil;
...
- (BOOL)buildVideoEffectsToMP4:(NSString *)exportVideoFile inputVideoURL:(NSURL *)inputVideoURL
{
    ...
    if (self.themeCurrentType != kThemeNone && [self.themesDic count] >= _themeCurrentType)
    {
        _themesDic = [[VideoThemesData sharedInstance]getThemesData];
    ...
    [self applyVideoEffectsToComposition:videoComposition size:videoLayer.bounds.size];
...
- (void)retrievingProgress
{
    if (_delegate && [_delegate respondsToSelector:@selector(retrievingProgressFilter:)])
    {
        [_delegate performSelector:@selector(retrievingProgressFilter:) withObject:[NSNumber numberWithFloat:_movieFile.progress]];
                     NSLog(@"Filter Progress: %f", _movieFile.progress);
    }
