...
#define THEME_H_PADDING   15.0f
#define THEME_H_START     12.0f
#define THEME_TAG_BASE    100

@interface ThemeFilterScrollView ()
...
@implementation ThemeFilterScrollView
...
- (void)initialize
{
    ...
    NSMutableDictionary *themImagesArray = [[VideoFilterData sharedInstance] getFilterData];
    NSMutableArray *temp = [NSMutableArray array];
    for (int i = kThemeFilterNone; i < [themImagesArray count]; ++i)
    {
...
- (void)setThemeHPadding:(CGFloat)themeHPadding
{
    ...
    if (count != 0)
    {
        ...
        addDis += (count * rise);
        [_themeScrollView setContentSize:CGSizeMake(contentSize.width + 10,self.bounds.size.height)];
    }
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(themeFilterScrollView:didSelectMaterial:)])
    {
        [self changeItemStateToNormal];
        self.selectedItem = themeImageView;
        [self.delegate themeFilterScrollView:self didSelectMaterial:[self.themeImages objectAtIndex:themeImageView.tag - THEME_TAG_BASE]];
    }
    else
    {
        [self changeItemStateToNormal];
...
- (void)loadThemeImages
{
    ...
    for (VideoThemes *matl in _themeImages)
    {
        ...
        {
            text = @"默认";
            thumbImage = @"filter_themeOriginal";
        }
        else
        {
            text = matl.name;
        ...
        {
            // 加载主题视图
            ThemeImageView *themeView = [[ThemeImageView alloc] initWithFrame:CGRectMake(0, 0, 70, 70)];
