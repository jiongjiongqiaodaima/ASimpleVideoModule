...
- (void)initialize
{
    ...
    NSMutableDictionary *themImagesArray = [[VideoThemesData sharedInstance] getThemesData];
...
- (void)initMusicVideo{
    ...
    self.themeHPadding  = THEME_H_PADDING;
    self.startXPosition = THEME_H_START;
    NSMutableDictionary *themImagesArray = [[VideoThemesData sharedInstance] getNewThemeDic];
    NSMutableArray *temp = [NSMutableArray array];
    for (int i = kThemeNone; i < [themImagesArray count]; ++i)
    {
        VideoThemes *material = [themImagesArray objectForKey:[NSNumber numberWithInt:i]];
