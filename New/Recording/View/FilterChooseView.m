...
-(UICollectionView *)collectionview
{
    if (!_collectionview) {
        ...
        [flowLayout setItemSize:CGSizeMake(self.bounds.size.width / 4, self.bounds.size.height)];
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        self.backgroundColor = [UIColor clearColor];
        _iconImg = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, self.bounds.size.height / 3 * 2 , self.bounds.size.height / 3 * 2)];
        _iconImg.clipsToBounds = YES;
        _iconImg.backgroundColor = [UIColor redColor];
        _iconImg.layer.cornerRadius = self.bounds.size.height / 3;
        _iconImg.layer.masksToBounds = YES;
        [self.contentView addSubview:_iconImg];
        
        _nameLab = [[UILabel alloc] initWithFrame:CGRectMake(0, self.bounds.size.height / 3 * 2 , self.bounds.size.height / 3 * 2 , self.bounds.size.height / 6 )];
        _nameLab.backgroundColor = [UIColor orangeColor];
        _nameLab.text = @"filter";
